// Heart animation module - wrapped for on-demand initialization
window.initHeartAnimation = async function() {
...
const particleCount = 3500; // Optimized for smooth performance
...
const themes = {
  cosmic: {
    ...
    bloom: { strength: 0.3, radius: 0.3, threshold: 0.7 } // Optimized for performance
  }
};
...
function init() {
  // Music is already handled by main script, no need to manage here
  ...
  // Show messages after particles form heart shape (3 seconds)
  setTimeout(() => {
    showMessages();
  }, 3000);
}
...
function showMessages() {
  ...
  // First message
  messageText.textContent = 'Chúc mừng sinh nhật Tú Anh,\nluôn may mắn và xinh đẹp nè';
...
window.addEventListener('startHeart', async () => {
  if (typeof window.initHeartAnimation === 'function' && !window.heartAnimationInitialized) {
    await window.initHeartAnimation();
    window.heartAnimationInitialized = true;
  }
});

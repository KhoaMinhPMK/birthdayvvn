// Wrap firework in a function to control initialization
window.initFirework = function() {
  'use strict';
  console.clear();
...
function getConfigFromDOM() {
    return {
        quality: appNodes.quality ? appNodes.quality.value : '3', // High quality for beautiful fireworks
        shell: appNodes.shellType ? appNodes.shellType.value : 'Random',
        size: appNodes.shellSize ? appNodes.shellSize.value : '3', // Large but not too much
        autoLaunch: appNodes.autoLaunch ? appNodes.autoLaunch.checked : true,
        finale: appNodes.finaleMode ? appNodes.finaleMode.checked : false, // Normal mode with pauses
        skyLighting: appNodes.skyLighting ? appNodes.skyLighting.value : '2', // Normal lighting
        longExposure: appNodes.longExposure ? appNodes.longExposure.checked : false,
        hideControls: appNodes.hideControls ? appNodes.hideControls.checked : true,
        // Store value as number.
        scaleFactor: appNodes.scaleFactor ? parseFloat(appNodes.scaleFactor.value) : 0.95 // Slightly smaller
    };
};
...
if (appNodes.helpModalCloseBtn) {
    appNodes.helpModalCloseBtn.addEventListener('click', () => {
        store.setState({ openHelpTopic: null });
    });
}

if (appNodes.helpModalOverlay) {
...
function init() {
    ...
    // Populate dropdowns (only if nodes exist)
...
function seqPyramid() {
    const barrageCountHalf = IS_DESKTOP ? 9 : 5; // Balanced pyramid
    const largeSize = shellSizeSelector();
    const smallSize = Math.max(0, largeSize - 3); // Normal small shells
...
function seqSmallBarrage() {
    ...
    const barrageCount = IS_DESKTOP ? 14 : 7; // Balanced barrage
    const specialIndex = IS_DESKTOP ? 4 : 2;
...
seqSmallBarrage.cooldown = 12000; // Longer cooldown for pauses
seqSmallBarrage.lastCalled = Date.now();
...
let isFirstSeq = true;
const finaleCount = 40; // Balanced finale
...
function startSequence() {
    ...
    if (rand < 0.12 && Date.now() - seqSmallBarrage.lastCalled > seqSmallBarrage.cooldown) {
        return seqSmallBarrage(); // Occasional barrages
    }
    
    if (rand < 0.2) {
        return seqPyramid(); // Occasional pyramids
    }
...
function updateGlobals(timeStep, lag) {
    ...
    if (store.state.config.autoLaunch) {
        ...
        if (autoLaunchTime <= 0) {
            autoLaunchTime = startSequence() * 1.0; // Balanced pace with pauses
        }
    }
}
...
// Skip loading UI and init directly
if (IS_HEADER) {
    init();
} else {
    // Preload assets and start app immediately (no loading screen needed)
    setTimeout(() => {
...
}; // End of initFirework function

// Auto-initialize when event is fired
window.addEventListener('startFirework', () => {
  if (typeof window.initFirework === 'function' && !window.fireworkInitialized) {
    window.initFirework();
    window.fireworkInitialized = true;
  }
});
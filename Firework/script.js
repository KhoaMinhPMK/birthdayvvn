...
// Continue playing background music from previous page
window.addEventListener('load', () => {
    ...
    if (music) {
        ...
        if (musicPlaying === 'true') {
            // Set the time to continue from where it left off
            if (musicTime) {
                music.currentTime = parseFloat(musicTime);
            }
            
            // Set volume to 50% (already faded in previous page)
            music.volume = 0.5;
    ...
    // Fade out black overlay after everything is loaded
    setTimeout(() => {
    ...
    }, 500); // Wait 500ms after load to ensure fireworks are ready
    
    // Stop fireworks after 30 seconds and fade to black
    setTimeout(() => {
        // Pause the fireworks
        store.setState({ paused: true });
        
        // Save music state before transition
        ...
        // After 2 seconds of black screen, navigate to heart page
        setTimeout(() => {
